...
{
    internal class ChatMagnager : MonoBehaviour
    {
        public static ChatMagnager Instance;
        //聊天列表相关组件
        [Header("聊天列表组件")]
        ...
        public static async void RefreshChatMessages(string chatId, List<ChatMessage> chatMessages)
        {
        ...
        public async void NotificeInviteResult(bool isSucceed, string name)
        {
            ...
            Debug.Log(text);
            ChatMagnager.Instance.NotificeShow(text);
            //如果添加成功则触发好友列表刷新
            if (isSucceed)
            {
                await Command.NetCommand.QueryAllChatTargetInfo();
